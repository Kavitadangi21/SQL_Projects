...
--using two tables tbl_book and tbl_book_issue
--union (used to fetch distinct records from both tables )
 select * from tbl_book
 union
 select * from tbl_book_issue

 --unionall (used to fetch all records from both tables (including  duplicates))
  select * from tbl_book
 union all
 select * from tbl_book_issue

 --intersect (to fetch common reccords  between both tables)
  select * from tbl_book
 interesect
 select * from tbl_book_issue

 --except (to fetch distinct records from 1st table which are not present in 2nd table)
  select * from tbl_book
 except
 select * from tbl_book_issue

 --joins
 --inner join(used to fetch records  which are common between both table based on common column)
  select b.book_id,b.book_edition,b.book_title,i.issue_date,i.return_date,i.issue_status
  from tbl_book as b
 inner join tbl_book_issue as i
 on b.book_id=i.book_id

 --leftjoin(to fetch records from left table and matching records from right table based on common field)
  select b.book_id,b.book_edition,b.book_title,i.issue_date,i.return_date,i.issue_status
  from tbl_book as b
 left join tbl_book_issue as i
 on b.book_id=i.book_id

 --rightjoin(to fetch records from right table and matching records from left table based on common field)
  select b.book_id,b.book_edition,b.book_title,i.issue_date,i.return_date,i.issue_status
  from tbl_book as b
 right join tbl_book_issue as i
 on b.book_id=i.book_id

 --fullouterjoin(to fetch all matching and non matching  records from both tables)
  select b.book_id,b.book_edition,b.book_title,i.issue_date,i.return_date,i.issue_status
  from tbl_book as b
 outer join tbl_book_issue as i
 on b.book_id=i.book_id

 --window function

  SELECT  * FROM tbl_book
	 SELECT book_id,book_title,book_edition , SUM(copies_total) OVER (partition by book_title ) AS newdata FROM tbl_book
	 
	 SELECT book_id,book_title,book_edition , MAX(copies_total) OVER (partition by book_title order by book_id) AS newdata FROM tbl_book
	 
	 SELECT book_id,book_title,book_edition , MIN(copies_total) OVER (order by book_id ) AS newdata FROM tbl_book

     SELECT  book_id,book_title,book_edition ,COUNT(book_title) OVER (partition by book_id) AS newdata FROM tbl_book

	 SELECT book_id,book_title,book_edition , RANK() OVER (order by book_id ) AS newdata FROM tbl_book

	 SELECT book_id,book_title,book_edition , ROW_NUMBER() OVER (order by book_id ) AS rowmum FROM tbl_book

     SELECT book_id,book_title,book_edition , DENSE_RANK() OVER (order by publication_year ) AS newdata FROM tbl_book

	 SELECT book_id,book_title,book_edition , percent_rank() OVER (order by book_id ) AS newdata FROM tbl_book

	SELECT book_id,book_title,book_edition , FIRST_VALUE(book_title) OVER (order by book_id ) AS newdata FROM tbl_book

	SELECT book_id,book_title,book_edition ,publication_year, LAST_VALUE(book_title) OVER (order by book_id ) AS newdata FROM tbl_book

	 SELECT book_id,book_title,book_edition , DENSE_RANK() OVER (order by publication_year)   AS dense, 
	 rank() OVER (order by publication_year) AS ranks, 
	 row_number() OVER (order by publication_year) AS rownum 
	   FROM tbl_book

	SELECT book_title,copies_total, LAG(copies_total) OVER (order by book_id ) AS newdata FROM tbl_book

	SELECT book_id, book_title ,copies_total,LEAD(copies_total,2) OVER (order by book_id ) AS newdata FROM tbl_book

	select book_id, lower(book_title) as low1 from tbl_book

--stringfunction

select *,upper(book_title) as upp_name from tbl_book

select *, lower(book_title) as low_name from tbl_book

select *, substring(book_title,2,5) as str from tbl_book

select len(category_id) as length from tbl_book

select *, concat(book_title,' ',book_edition,' ',book_id) as book_details from tbl_book

select *, reverse(book_title) as rev_name from tbl_book

select * ,replicate(book_title,3) as repeated_name from tbl_book

select  book_title, replace('book_title' ,'book_title','physics') as data1 from tbl_book
where book_id=1

select trim('    book   ') as trimmed 
--same we use ltrim and rtrim to remove trailing and leading spaces

select charindex('gory', 'category_id') as position

select string_agg(book_title,'-') as new_value from tbl_book 

select ceiling(134.55) as positive

select floor(134.55) as dec

select round(3440684.58495,2)


--alter command used to addcolumn ( add new columns to table,or to add multiple columns to same table we have to give column name's separated with comma)
 ...
 --addconstraint
  alter table tbl_book
 add CONSTRAINT cnstrnt1 unique(book_name)

 --delete (used to delete all columns or specified column using where condition from table)
 delete from tbl_book
 where book_title='Gilu'

 --truncate command used to delete all rows from table except structure of table(here tbl_book has pk and fk columns so it not  deleted)  
 truncate table tbl_name

--drop database
  drop database [lib_mngmt_system]

--Drop tables( only using one table)
  Drop table tbl_book
